...
static inline unsigned int pd_header_msgid(uint16_t header)
{
...
static inline unsigned int pd_header_rev(uint16_t header)
{
...
static inline unsigned int pd_ext_header_chunk_num(uint16_t ext_header)
{
	return (ext_header >> PD_EXT_HDR_CHUNK_NUM_SHIFT) &
		PD_EXT_HDR_CHUNK_NUM_MASK;
}

static inline unsigned int pd_ext_header_data_size(uint16_t ext_header)
{
...
struct pd_chunked_ext_message_data {
	__le16 header;
	uint8_t data[PD_EXT_MAX_CHUNK_DATA];
} __packed;
...
#define PDO_FIXED_VOLT(mv)	((((uint32_t)(mv) / 50) & PDO_VOLT_MASK) << PDO_FIXED_VOLT_SHIFT)
#define PDO_FIXED_CURR(ma)	((((uint32_t)(ma) / 10) & PDO_CURR_MASK) << PDO_FIXED_CURR_SHIFT)

#define PDO_FIXED(mv, ma, flags) 0 | flags | PDO_FIXED_VOLT(mv) | PDO_FIXED_CURR(ma)
/*
...
//XC8 does not default to 32-bit constants -> shift by 17 causes compiler error; multiply 131072 instead
#define PDO_PPS_APDO(min_mv, max_mv, max_ma, flags)  flags | 0xc0000000  | (((min_mv / 100) & 0x000000ff)<< 8) | (((max_mv / 100) & 0x000000ff) * 131072) | (((max_ma / 50) & 0x7f) << 0)
...
static inline enum pd_apdo_type pdo_apdo_type(uint32_t pdo)
{
...
static inline unsigned int rdo_op_power(uint32_t rdo)
{
	return ((rdo >> RDO_BATT_OP_PWR_SHIFT) & RDO_PWR_MASK) * 250;
}

static inline unsigned int rdo_max_power(uint32_t rdo)
{
...
#define PD_T_SENDER_RESPONSE	29	/* 24 - 30 ms, relaxed. Changed from 26 to pass Lecroy */
#define PD_T_SOURCE_ACTIVITY	45
#define PD_T_SINK_ACTIVITY	135
#define PD_T_SINK_WAIT_CAP	240
#define PD_T_PS_TRANSITION	500
#define PD_T_SRC_TRANSITION	25
...
#define PD_T_CC_DEBOUNCE	150	/* 100 - 200 ms */
#define PD_T_PD_DEBOUNCE	15	/* 10 - 20 ms */
